<template>
  ...
  >
    ...
    <div class="protocol">
      ...
      <input
        class="protocol__input"
        type="number"
        name="protocolo"
        id="protocolo"
        v-model="formData.protocolo"
        placeholder="Número do protocolo"
        required="true"
      />
      <p hidden>
        <label>
          Não preencha esse campo se você for um humano:
...
export default {
  ...
  methods: {
    ...
    handleSubmit(e) {
      if (
        this.formData.opcao === "resultado" &&
        this.formData.protocolo != ""
      ) {
        window.open(
          "https://netpacs.fonteimagem.com.br/Netpacs-paciente/#/login-externo/" +
            this.formData.protocolo,
          "_blank"
        );
      } else {
